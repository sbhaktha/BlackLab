...
public class QueryExecutionContext {
	...
	private String[] getAlternatives() {
		...
		ComplexFieldProperty.SensitivitySetting sensitivity = pd.getSensitivity();
...
//		}
		...
		if (!caseSensitive && !diacriticsSensitive) {
			...
			if (sensitivity != ComplexFieldProperty.SensitivitySetting.ONLY_SENSITIVE)
				validAlternatives.add(i);
			if (sensitivity != ComplexFieldProperty.SensitivitySetting.ONLY_INSENSITIVE)
			...
			if (sensitivity == ComplexFieldProperty.SensitivitySetting.CASE_AND_DIACRITICS_SEPARATE)
